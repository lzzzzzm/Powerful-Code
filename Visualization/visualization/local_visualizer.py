...
class Det3DLocalVisualizer(DetLocalVisualizer):
    ...
    def _draw_pts_sem_seg(self,
        ...
        if ignore_index is not None:
            ignore_index = np.where(pts_sem_seg!=ignore_index)[0]
            ignore_index = tensor2ndarray(ignore_index)
            points = points[ignore_index]
            pts_sem_seg = pts_sem_seg[ignore_index]
    ...
    def _draw_occ_sem_seg(self,
        ...
        points, occ_voxel = voxel2points(occ_seg, ignore_labels=ignore_labels, voxelSize=voxelSize, range=range)
        points = points.numpy()
        ...
        bboxes = voxel_profile(torch.tensor(points), voxel_size=voxelSize)
